...
test('les ressources importantes ouvrent leurs liens exacts depuis la recherche', async ({ page }) => {
  ...
  const exactLinks = {
    locaux: 'https://drive.google.com/file/d/13dpfWN2_Jws3V4bwptfamOe-VSahZvCd/view?usp=drivesdk',
    enseignants: 'https://drive.google.com/file/d/1ptDonxP2Dx4FVUmY36PlppBYDc0HVRG5/view?usp=drivesdk',
    dineurs: 'https://drive.google.com/file/d/1CWkHvJRbFQMjYAkBs26e5vMPf83FbGVJ/view?usp=drivesdk',
  ...
  const existing = await page.evaluate(() => {
    const findHref = (id, text) => {
      ...
      const link = [...(node?.querySelectorAll('.procedure-content a[href^="http"]') || [])].find(a =>
        a.textContent.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().includes(wanted)
      );
      return link?.href || '';
    };
    return {
      locaux: findHref('horaire-locaux-2026-2027', 'horaire des locaux'),
      enseignants: findHref('horaire-enseignants-2026-2027', 'horaire des enseignants'),
      dineurs: findHref('horaires-surveillance-2026-2027', 'surveillance des dineurs'),
      bibliotheque: findHref('horaires-surveillance-2026-2027', 'surveillance bibliotheque')
    };
  });
