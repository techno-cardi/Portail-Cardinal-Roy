alter table public.portal_analytics_events
  drop constraint if exists portal_analytics_events_event_type_check;

alter table public.portal_analytics_events
  add constraint portal_analytics_events_event_type_check
  check (event_type in ('search','open','visit'));
...
period_totals as (
  select
    count(*) filter (where event_type = 'visit' and created_at >= current_date)::integer as visits_today,
...
comment on function public.portal_analytics_summary(integer) is
  'Résumé agrégé et sans texte de recherche pour le tableau admin public non indexé.';
